...
namespace GamingStoreTest
{
    public class GamingControllerTests
    {
        ...
        public async Task AddGameTag_OK()
        {
            //Setup
            var request = new AddGameTagRequest
            {
                Title = "League Of Legends",
                GameTag = "Action"
            };
            ...
            mockedGamesRepository.Setup(x => x.AddGameTag(request.Title, request.GameTag)).Callback(() =>
           {
               var game = GamesData.FirstOrDefault(x => x.Title.Contains(request.Title));
               if (game != null)
               {
                   game.GameTags = $"{game.GameTags}, {request.GameTag}";
               }
           });
            ...
            await gamesController.AddGameTag(request);

            //Assert

            var game = GamesData.FirstOrDefault(x => x.Title.Contains(request.Title));
            game.GameTags.Should().Contain(request.GameTag);
        }

        [Fact]

        public async Task AddGameTag_GameNotFound()
        {
            //Setup
            var request = new AddGameTagRequest
            {
        ...
        public async Task RemoveGameTag_OK()
        {
            //Setup
            var request = new RemoveGameTagRequest
            {
                Title = "League Of Legends",
                GameTag = "Moba"
            };
            ...
            mockedGamesRepository.Setup(x => x.RemoveGameTag(request.Title, request.GameTag)).Callback(() =>
            {
                var game = GamesData.FirstOrDefault(x => x.Title.Contains(request.Title));
                if (game != null)
                {
                    var index = game.GameTags.IndexOf(request.GameTag);
                    if (index < 1)
                    {
                        game.GameTags = game.GameTags.Replace($"{ request.GameTag}, ", "");
                    } 
                    else
                        game.GameTags = game.GameTags.Replace($", {request.GameTag}", "");                    
                }
            ...
            await gamesController.RemoveGameTag(request);

            //Assert

            var game = GamesData.FirstOrDefault(x=>x.Title.Contains(request.Title));
            game.GameTags.Should().NotContain(request.GameTag);
        }

        [Fact]

        public async Task RemoveGameTag_GameNotFound()
        {
            //Setup
            var request = new RemoveGameTagRequest
            {
        ...
        public async Task RemoveGameTag_GameTagNotFound()
        {
            //Setup
            var request = new RemoveGameTagRequest
            {
            ...
            mockedGamesRepository.Setup(x => x.RemoveGameTag(request.Title, request.GameTag)).Callback(() =>
            {
                ...
                if (game != null)
                {
                    if (game.GameTags.Contains(request.GameTag))
                    {
                        if (game != null)
                        {
                            var index = game.GameTags.IndexOf(request.GameTag);
